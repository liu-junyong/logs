...
// InfoV2 ...
func (l *Logger) InfoV2(msg string) {
	if LevelInfo < l.level {
		return
	}
	l.fmtLog(nil, LevelInfo, msg)
}
...
func (l *Logger) prefixV1(ctx context.Context, level int, writer io.Writer) {
	...
	if l.callDepth > 0 {
		...
		_, _ = file, line
		writer.Write([]byte(filepath.Base(file)))
		writer.Write([]byte{':'})
		writer.Write([]byte(strconv.Itoa(line)))
		writer.Write(spaceBytes)
	}
...
func (l *Logger) WriteKvs(level int, ip, psm, logid, cluster string, kvs ...interface{}) {
	if level < l.level {
		return
	}
	if atomic.LoadInt32(&l.isRunning) == 0 {
